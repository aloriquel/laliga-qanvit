/**
 * Imports the CDTI NEOTEC dataset from
 * research/scrapers/cdti-neotec/output/cdti_neotec_dataset_v1.json
 * into public.award_recipients (and award_editions).
 *
 * Mirrors scripts/import_south_summit.ts. Idempotent via clean-slate
 * DELETE scoped to this award's editions, then plain INSERT.
 *
 * Pre-requisitos:
 *   - .env.local con NEXT_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY.
 *   - Dataset cdti_neotec_dataset_v1.json generado por
 *     research/scrapers/cdti-neotec/scrape.py.
 *   - Migrations 0060 + 0061 aplicadas (beneficiary enum,
 *     award_amount_eur + company_cif columns, award seed).
 *
 * Usage:
 *   npx tsx scripts/import_cdti_neotec.ts
 */
...
async function main() {
  ...
  if (!existsSync(datasetPath)) {
    console.error(
      ...
      "\nRun:  cd research/scrapers && python cdti-neotec/scrape.py --phase=all"
    );
    process.exit(1);
  }
  ...
  const { data: award } = await sb
    ...
    .single();
  if (!award) {
    throw new Error("award 'cdti-neotec' not found — apply migration 0061");
  }
  ...
  const editionRows = [...editionKeySet].map((k) => {
    ...
    return {
      ...
      edition_number: Number(year) - 2001, // 2002 = 1st edition.
      category_type: ct,
      category_value: cat,
      source_url:
        data.find(
  ...
  const recipientRows = data.map((r) => {
    ...
    let desc = r.company_description_short || r.project_title || null;
    if (desc && desc.length > 240) desc = desc.slice(0, 239) + "…";
    return {
      edition_id: editionId,
      result: r.result === "beneficiary" ? "beneficiary" : "winner",
      ...
      external_id: r.id,
      is_spanish_ecosystem: r.is_spanish_ecosystem !== false,
      company_country: r.company_country ?? "Spain",
      company_cif: r.company_cif ?? null,
      award_amount_eur: r.award_amount_eur ?? null,
    };
  });

  // Clean slate for cdti-neotec.
  ...
  console.log(`Deleted ${delCount ?? 0} prior cdti-neotec recipients (clean slate)`);
