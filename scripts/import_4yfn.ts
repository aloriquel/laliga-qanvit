/**
 * Imports the 4YFN Awards dataset from research/scrapers/4yfn/output/4yfn_dataset_v1.json
 * into public.award_recipients (and award_editions).
 *
 * Reuses the SECURITY DEFINER `_tmp_admin_exec_sql` pattern only briefly,
 * but here we use the standard supabase-js client with service_role
 * (read from .env.local) and per-row INSERTs via a helper RPC. Simpler path:
 * load the JSON, generate INSERT VALUES, run them through the
 * service_role-authenticated REST endpoint via supabase.rpc.
 *
 * Idempotent: ON CONFLICT (external_id) DO NOTHING via per-row constraint.
 *
 * Usage:
 *   npx tsx scripts/import_4yfn.ts
 *
 * Pre-requisitos:
 *   - .env.local con NEXT_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY.
 *   - Dataset 4yfn_dataset_v1.json generado por research/scrapers/4yfn/scrape.py.
 *   - Migrations 0056/0057/0058 aplicadas.
 */
...
type RawRecord = {
  ...
  scraped_at?: string;
  id: string; // external_id from the CSV/JSON
};
...
async function main() {
  ...
  if (!existsSync(datasetPath)) {
    console.error(
      ...
      "\nRun:  cd research/scrapers && python 4yfn/scrape.py --phase=all"
    );
    process.exit(1);
  }
  ...
  const { data: award } = await sb
    ...
    .eq("slug", "4yfn")
    .single();
  if (!award) throw new Error("award '4yfn' not found — apply migration 0058");
  ...
  const editionRows = [...editionKeySet].map((k) => {
    ...
    return {
      ...
      edition_number: Number(year) - 2013,
      category_type: ct,
      category_value: cat,
      source_url: data.find(
        (r) => r.edition_year === Number(year) && r.category_value === cat
      )?.source_url ?? null,
    };
  });

  // Upsert editions in chunks of 100.
