...
AS $$
DECLARE
  fn_url    text := get_setting('evaluator_url');
  fn_secret text := get_setting('evaluator_secret');
BEGIN
  IF fn_url IS NULL OR fn_url = '' OR fn_secret IS NULL OR fn_secret = '' THEN
    RAISE WARNING 'trigger_evaluator_pipeline: settings not configured, skipping deck %', NEW.id;
    RETURN NEW;
  END IF;
...
-- Settings read: alert_dispatcher_url, evaluator_secret
...
AS $$
DECLARE
  fn_url    text := get_setting('alert_dispatcher_url');
  fn_secret text := get_setting('evaluator_secret');
BEGIN
  IF fn_url IS NULL OR fn_url = '' THEN
    RAISE WARNING 'notify_alert_dispatcher: settings not configured, skipping alert %', NEW.id;
    RETURN NEW;
  END IF;
...
-- Settings read: ecosystem_alert_dispatcher_url, evaluator_secret
...
AS $$
DECLARE
  fn_url    text := get_setting('ecosystem_alert_dispatcher_url');
  fn_secret text := get_setting('evaluator_secret');
  is_first  boolean;
BEGIN
  ...
  IF fn_url IS NULL OR fn_url = '' OR fn_secret IS NULL OR fn_secret = '' THEN
    RAISE WARNING 'notify_ecosystem_alert_dispatcher: settings not configured, skipping evaluation %', NEW.id;
    RETURN NEW;
  END IF;
